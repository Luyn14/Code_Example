...
CREATE TABLE tCrew (
	...
	CrewroleFID INT,
	PRIMARY KEY (CrewID),
	FOREIGN KEY (CrewroleFID) REFERENCES tCrewrole(CrewroleID)
);
...
CREATE TABLE tDivision (
	...
	FOREIGN KEY (FleetFID) REFERENCES tFleet(FleetID)
);
...
CREATE TABLE tDest_Moon (
	...
	FOREIGN KEY (DestinationFID) REFERENCES tDestination(DestinationID),
	FOREIGN KEY (MoonFID) REFERENCES tMoon(MoonID)
);
...
CREATE TABLE tSpaceship (
	...
	FOREIGN KEY (DivisionFID) REFERENCES tDivision(DivisionID),
	FOREIGN KEY (SpaceshiproleFID) REFERENCES tSpaceshiprole(SpaceshiproleID),
	FOREIGN KEY (DestmoonFID) REFERENCES tDest_Moon(DestmoonID)
); 
...
CREATE TABLE tSpaceship_Shipequipment (
    ...
    FOREIGN KEY (SpaceshipFID) REFERENCES tSpaceship(SpaceshipID),
	FOREIGN KEY (ShipequipmentFID) REFERENCES tShipequipment(ShipequipmentID)
);
...
CREATE TABLE tCrew_Spaceship (
	...
	FOREIGN KEY (CrewFID) REFERENCES tCrew(CrewID),
	FOREIGN KEY (SpaceshipFID) REFERENCES tSpaceship(SpaceshipID)
);
...
CREATE TABLE tSpaceship_Cargo (
	...
	FOREIGN KEY (SpaceshipFID) REFERENCES tSpaceship(SpaceshipID),
	FOREIGN KEY (CargoholdFID) REFERENCES tCargohold(CargoholdID)
);
...
CREATE TABLE tSpaceship_Ammunition (
	...
	FOREIGN KEY (SpaceshipFID) REFERENCES tSpaceship(SpaceshipID),
	FOREIGN KEY (AmmunitionholdFID) REFERENCES tAmmunitionhold(AmmunitionholdID)
);
...
INSERT INTO tCrew 
	(CrewID, Prename, Lastname, Birthday, CrewroleFID)
	VALUES
	(1, "Luca", "Luu", "23.11.75", 1),
	(2, "Michael", "Frantzes", "9.1.77", 2),
	(3, "Donald", "Tate", "30.3.81", 2),
	(4, "Ryan", "King", "10.12.74", 3),
	(5, "Roland", "Sherman", "23.11.72", 3),
	(6, "Nick", "Lauda", "11.5.83", 3),
	(7, "Nedo", "Lui", "3.7.69", 3);
